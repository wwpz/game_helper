...
class PPOCR_pipe:  # 调用OCR（管道模式）
    ...
    def runDict(self, writeDict: dict, show_log=True):
        """传入指令字典，发送给引擎进程。\n
        `writeDict`: 指令字典。\n
    ...
    def runBase64(self, imageBase64: str, show_log=True):
        """对一张编码为base64字符串的图片进行文字识别。\n
        `imageBase64`: 图片base64字符串。\n
        `return`:  {"code": 识别码, "data": 内容列表或错误信息字符串}\n"""
        writeDict = {"image_base64": imageBase64}
        return self.runDict(writeDict, show_log)

    def runBytes(self, imageBytes, show_log=True):
        """对一张图片的字节流信息进行文字识别。\n
        `imageBytes`: 图片字节流。\n
        `return`:  {"code": 识别码, "data": 内容列表或错误信息字符串}\n"""
        imageBase64 = b64encode(imageBytes).decode("utf-8")
        return self.runBase64(imageBase64, show_log)
